...
def _val_synonyms_prompt(col: str, vals: list[str], domain: str) -> str:
    ...
    return (
        f"Ты работаешь с колонкой «{col}» в таблице из домена: «{domain}».\n"
        f"Для каждого значения придумай ровно {N_VAL_SYNS} альтернативных варианта.\n"
        "Варианты могут быть: перевод, синоним, сокращение, другой регистр, другой формат.\n\n"
        f"Значения:\n{val_list}\n\n"
        'Ответь ТОЛЬКО JSON объектом вида:\n'
        '{"значение": ["вар1", "вар2", "вар3"], ...}\n'
        "Без пояснений. Только JSON."
    )
