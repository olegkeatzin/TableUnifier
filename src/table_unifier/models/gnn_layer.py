...
EdgeMeanConv: Linear(concat(h_j, edge_attr)) + mean aggregation
...
class GNNLayer(nn.Module):
    ...
    def __init__(
        ...
        # Token → Row
        self.conv_t2r = EdgeMeanConv(hidden_dim, edge_dim, hidden_dim)
        self.norm_row = nn.LayerNorm(hidden_dim)
        self.dropout = nn.Dropout(dropout)

        # Row → Token (опционально)
        if bidirectional:
            self.conv_r2t = EdgeMeanConv(hidden_dim, edge_dim, hidden_dim)
