...
function ScreenResult({ onBack, onRestart, decisions }) {
  ...
  return (
    <div className="screen">
      ...
      <ScreenFooter onBack={onBack} onNext={onRestart} nextLabel="Новый pipeline">
...
function UnifiedTable({ clusters, expanded, toggle }) {
  ...
  if (!tblA || !tblB) {
    return <div style={{ padding: 24, color: 'var(--text-4)' }}>Нет данных. Запустите inference.</div>;
  }
  ...
  const canonical = (c) => {
    ...
    return Object.fromEntries(cols.map((col) => {
      ...
      if (values.every((v) => typeof v === 'number' || (!isNaN(parseFloat(v)) && isFinite(v)))) {
        ...
        return [col, (nums.reduce((a, b) => a + b, 0) / nums.length).toFixed(2)];
      }
      // mode
      const counts = {};
