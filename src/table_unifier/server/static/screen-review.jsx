    ...
    <div className="screen">
      <div className="screen-header">
        <div>
          ...
          <p>GNN выдал {allCandidates.length} пар. Connected components автоматически разрешили {counts.auto}; {counts.review} требуют ручной проверки.</p>
        </div>
      </div>
      ...
      <div className="screen-body" style={{ display: 'grid', gridTemplateColumns: '300px 1fr', minHeight: 0 }}>
        <div style={{ borderRight: '1px solid var(--border)', padding: '14px 14px 0', display: 'flex', flexDirection: 'column', minHeight: 0 }}>
          ...
          <div style={{ marginTop: 20 }}>
            <div style={{ fontSize: 10.5, color: 'var(--text-4)', textTransform: 'uppercase', letterSpacing: 0.06, marginBottom: 8 }}>similarity threshold</div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 4, fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--text-4)' }}>
              <span>0.50</span><span>thr</span><span>0.99</span>
            </div>
          </div>

          <div style={{ marginTop: 18, padding: 12, background: 'var(--bg-elev)', border: '1px solid var(--border)', borderRadius: 8 }}>
            <div style={{ fontSize: 10.5, color: 'var(--text-4)', textTransform: 'uppercase', letterSpacing: 0.06, marginBottom: 6 }}>clustering</div>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--text-2)', lineHeight: 1.6 }}>
              <div><span style={{ color: 'var(--text-4)' }}>method:</span> connected components</div>
              <div><span style={{ color: 'var(--text-4)' }}>metric:</span> cosine</div>
              <div><span style={{ color: 'var(--text-4)' }}>threshold:</span> {Number(D.metrics?.threshold ?? 0.831).toFixed(3)}</div>
            </div>
          </div>
      ...
      <ScreenFooter
        ...
        nextDisabled={false}>
        {pendingReview > 0
          ? <>Осталось {pendingReview} пар. Можно продолжить — нерешённые остаются с авто-вердиктом.</>
          : <>Всё проверено.</>}
      </ScreenFooter>
    </div>
  );
...
function PairCard({ cand, decision, onDecide }) {
  const D = window.__DATA__;
      ...
      <div className="pair-h">
        ...
        <div className="right">
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--text-3)' }}>
            sim = <b style={{
              color: sim > 0.85 ? 'var(--cluster)' : sim > 0.7 ? 'var(--warn)' : 'var(--bad)',
              fontSize: 13,
            }}>{sim.toFixed(3)}</b>
          </div>
      ...
      <div className="pair-body">
        <div className="pair-side">
          <h5>
            ...
            <span style={{ color: 'var(--text-4)' }}>· row #{cand.a[1] + 1}</span>
          </h5>
          {colsA.map((col, i) => (
